...
np.where(np.logical_and(results[51]>0, results[51]<0.5))


# In[54]:


plt.plot(np.abs(vecs[51,600]))
chunk_size = len(vecs[51, 600])//9
for div in np.arange(10):
    plt.axvline(chunk_size*div, ls=':', c='k')


# In[70]:


plt.plot(np.imag(vecs[51,600]),'.', ms=2, label='Im(psi)')
plt.plot(np.real(vecs[51,600]),'.', ms=2, label='Re(psi)')
chunk_size = len(vecs[51, 600])//9
for div in np.arange(10):
    plt.axvline(chunk_size*div, ls=':', c='k')
plt.legend()
plt.xlim(-1,len(vecs[51,600]))


# In[42]:


magic_vec = vecs[51, 600]


# In[51]:


plt.plot(k_vals[45:60], results[45:60,600])
plt.plot(k_vals[51], results[51,600], '.k')
