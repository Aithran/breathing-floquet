...
resid_file_path = os.path.join(base_path, 'drive_m20_all_residuals.h5')
...
print(resid_file_path)
with h5py.File(resid_file_path, 'r', libver="latest") as f:
    full_scan = f['total_residuals']
...
plt.imshow(np.real(resorted[2620:2660]).T, vmin=-1, vmax=1, cmap='RdBu', aspect=0.1)
