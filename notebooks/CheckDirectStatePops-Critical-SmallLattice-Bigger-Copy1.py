...
base_path = base_dir
data_dirs = [os.path.basename(dirpath[:-1]) for dirpath in glob.glob(base_path + "/*_big/")]
print(data_dirs)
...
plt.plot(np.sort(np.abs(first_set_residuals_dict[128]))/n_time_samples)
plt.plot(np.sort(np.abs(last_set_residuals_dict[128]))/n_time_samples)
...
result_file_path = os.path.join(base_path, 'residual_pops_critical_128.h5')
...
resid_file_path = os.path.join(base_path, 'residual_pops_critical_128.h5')
print(resid_file_path)
#raise RuntimeException("Don't overwrite things yet")
