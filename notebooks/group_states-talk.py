...
talk_style = {
    'lines.linewidth': LINE_SIZE,
    'axes.linewidth': 0.5*LINE_SIZE,
    'grid.linewidth': LINE_SIZE,
    'font.family': 'serif',
    'font.serif': 'Computer Modern Roman',
    'text.usetex': True,
...
umd_small_cxt = mpl.rc_context(rc=talk_style)
...
out_dir = os.path.join(os.getcwd(), 'talk_plots')
out_ext = '.png'
out_dpi = 220
print(out_dir)
...
roughLess, refinedLess, sourceLess = load_resid_file(data_paths[1])
print(sourceLess)
roughMore, refinedMore, sourceMore = load_resid_file(data_paths[4])
print(sourceMore)
...
with mpl.rc_context(rc=talk_style):
    fig, ax_arr = plt.subplots(1,2, sharex=True,sharey=True,figsize=(5.5,5.5))
    ...
    ax_arr[0].set_xlim(1e-16,1)
    ax_arr[1].set_xlim(1e-16,1)
    fig.tight_layout()
    #fig.savefig('deltadelta.pdf', dpi=600, bbox_inches='tight', pad_inches=1/72)
    plt.pause(0.1)
    fig.savefig(os.path.join(out_dir, 'deltadelta' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
    plt.pause(0.1)
...
this_zero_diff = []
this_diff_split = np.NaN
this_last_diff = []
def find_fbz_qes(resids, source_path, resid_thresh=5e-14, diff_split=1e-6,
                 stride=1, offset=0):
    global n_sites, m_rad, n_ms, n_states, this_zero_diff, this_diff_split, this_last_diff
    with h5py.File(source_path, 'r', libver='latest') as f:
        ...
        for set_idx in progbar(np.arange(0,n_sets)):
            ...
            if set_idx == 0:
                ...
                this_zero_diff = np.diff(sorted_en)
                plt.axhline(diff_split, c='grey', ls=':', lw=1)
                this_diff_split = diff_split
            n_deg = 0
        ...
        this_last_diff = np.diff(sorted_en)
...
def find_random_baddie_qes(resids, source_path, resid_thresh=1e-1, stride=1, offset=0):
    with h5py.File(source_path, 'r', libver='latest') as f:
        print(os.path.basename(source_path))
        n_sites = f.attrs['n_sites']
        m_rad = f.attrs['floquet_m_radius']
        ...
        all_fqe_count = np.empty_like(all_fqe_means)
        all_fqe_minmax = np.empty((n_sets, n_sites, 2))
    return stride*(n_sets-1) + offset, np.random.choice(np.where(resids[n_sets-1] > resid_thresh)[0])


# In[320]:


get_ipython().run_cell_magic('time', '', 'set_baddie, random_baddie = find_random_baddie_qes(refinedMore, sourceMore, stride=2, resid_thresh=2)\nn_ts = 1024\nprint(random_baddie)\nwith h5py.File(sourceMore, \'r\', libver=\'latest\') as f:\n    print(os.path.basename(sourceMore))\n    state_baddie = f[\'floquet_state\'][set_baddie, :, random_baddie]\n    n_sites_baddie = f.attrs[\'n_sites\']\n    m_rad_baddie = f.attrs[\'floquet_m_radius\']\nprint(state_baddie.shape, n_sites_baddie*(2*m_rad_baddie+1))\ndef get_cycle_proj_op(n_states, n_floquet):\n    return np.tile(np.diag(np.ones(n_states)), (1, n_floquet))\ncompress = get_cycle_proj_op(n_sites_baddie, 2*m_rad_baddie+1)\ndef time_phasor(t_over_T, scaled_quasi_energy=1000):\n    return np.exp(np.complex(0,2.0*np.pi) * t_over_T * (np.repeat(np.arange(-m_rad_baddie, m_rad_baddie+1), n_sites_baddie) - scaled_quasi_energy))\ndef site_pop_at_time(t_over_T, floquet_state):\n    return compress @ np.diag(time_phasor(t_over_T)) @ floquet_state\n#one_cycle_trace = np.empty((n_ts,n_sites_baddie), dtype=np.complex)\none_cycle_totals = np.empty((n_ts,n_sites_baddie))\nts = np.linspace(0.0,1.0, n_ts)\nfor idx, t in progbar(enumerate(ts), size=n_ts, every=8):\n    one_cycle_totals[idx] = np.sum(np.square(np.abs(site_pop_at_time(t, state_baddie[:, np.newaxis]).flatten())))\nwith mpl.rc_context(rc=talk_style):\n    fig, ax_array = plt.subplots(nrows=1, ncols=1, sharex=False, figsize=(8,4))\n    ax_array.axhline(1.0, c=\'r\', ls=\':\')\n    ax_array.plot(ts, one_cycle_totals, \'k\', label="Total")\n    ax_array.set_xlabel(r\'$t/T$\')\n    ax_array.set_ylabel(r\'$p_\\mathrm{anywhere}(t)$\')\n    ax_array.set_xlim(0,1)\n    ax_array.set_ylim(0,9)\n    plt.pause(0.1)\n    fig.savefig(os.path.join(out_dir, \'p_anywhere\' + out_ext), dpi=out_dpi, bbox_inches=\'tight\', pad_inches=1/72)\n    plt.pause(0.1)')
...
with mpl.rc_context(rc=talk_style):
    ...
    ax_array.set_ylabel(r'$p_\mathrm{anywhere}(t)$')
    ax_array.set_xlim(0,1)
    ax_array.set_ylim(-0.2,8.1)
    ax_array.axhline(0.0, c='k', lw=0.5)
    plt.pause(0.1)
    fig.savefig(os.path.join(out_dir, 'p_anywhere' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
    plt.pause(0.1)
print(np.mean(np.square(one_cycle_totals-1)))
...
with mpl.rc_context(rc=talk_style):
    fig, ax_array = plt.subplots(nrows=1, ncols=1, sharex=False, figsize=(8,4))
    ax_array.axhline(1.0, c='r', ls=':')
    ax_array.plot(ts, np.square(one_cycle_totals-1), 'k', label="Total")
    ax_array.set_xlabel(r'$t/T$')
    ax_array.set_ylabel(r'$p_\mathrm{anywhere}(t)$')
    ax_array.set_xlim(0,1)
    ax_array.set_ylim(-1,45)
    ax_array.axhline(0.0, c='k', lw=0.5)
    plt.pause(0.1)
    #fig.savefig(os.path.join(out_dir, 'p_anywhere' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
    plt.pause(0.1)
...
fbz_zero_diff = this_zero_diff.copy()
fbz_diff_split = this_diff_split
fbz_last_diff = this_last_diff.copy()
plt.ylim(None,1)
...
with mpl.rc_context(rc=talk_style):
    ...
    ax = plt.subplot(111)
    ax.semilogy(fbz_zero_diff, '.', ms=LINE_SIZE, c=kelly_color_cycler.by_key()['color'][0])
    ax.semilogy(fbz_last_diff, '.', ms=LINE_SIZE, c=kelly_color_cycler.by_key()['color'][1])
    ax.axhline(fbz_diff_split, c='grey', ls=':', lw=LINE_SIZE)
    ax.set_ylim(None,None)
    ax.set_xlim(0,len(fbz_zero_diff))
    ax.set_xticks([])
    ax.set_ylabel('$(\epsilon_{i+1} - \epsilon_{i})/h$ (Hz)')
    plt.pause(0.1)
    fig.savefig(os.path.join(out_dir, 'quasi_diffs_45' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
    plt.pause(0.1)
...
fbz_n33_zero_diff = this_zero_diff
fbz_n33_diff_split = this_diff_split
fbz_n33_last_diff = this_last_diff
plt.ylim(None,1)
...
with mpl.rc_context(rc=talk_style):
    fig = plt.figure(figsize=(6,6))
    ax = plt.subplot(111)
    ax.semilogy(fbz_n33_zero_diff, '.', ms=LINE_SIZE, c=kelly_color_cycler.by_key()['color'][0])
    ax.semilogy(fbz_n33_last_diff, '.', ms=LINE_SIZE, c=kelly_color_cycler.by_key()['color'][1])
    ax.axhline(fbz_n33_diff_split, c='grey', ls=':', lw=LINE_SIZE)
    ax.set_ylim(None,None)
    ax.set_xlim(0,len(fbz_n33_zero_diff))
    ax.set_xticks([])
    ax.set_ylabel('$(\epsilon_{i+1} - \epsilon_{i})/h$ (Hz)')
    time.sleep(1)
    fig.savefig(os.path.join(out_dir, 'quasi_diffs_33' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
...
with mpl.rc_context(rc=talk_style):
    fig, ax_arr = plt.subplots(2,2, sharey=True, figsize=(5.5,4.5))
    ax_arr[0,0].set_ylabel(r"$E/h$ (Hz)")
    ...
    ax_arr[1,1].set_xlabel(r"$\gamma$ (Breathing Amp.)")
    ax_arr[1,0].semilogx(n33_gammas[::], fbz_n33_mean[:,:], 'k', lw=1);
    ax_arr[1,1].set_xlim(1e-5,1e-1)
    ax_arr[1,0].set_xlim(1e-5,1e-1)
    ax_arr[1,0].set_xlabel(r"$\gamma$ (Breathing Amp.)")
    ax_arr[1,0].set_ylabel(r"$\epsilon/h$ (Hz)")
    ax_arr[0,0].set_title(r'$N_\mathrm{sites}=33$')
    ax_arr[0,1].set_title(r'$N_\mathrm{sites}=45$')
    plt.tight_layout()
    fig.savefig(os.path.join(out_dir, 'quasien_lattice' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/36)
...
with umd_small_cxt:
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'x_states' + out_ext), dpi=600, bbox_inches='tight', pad_inches=1/72)
...
maxmag33 = np.amax(np.abs(heff33))
plt.imshow(heff33, cmap="PuOr", vmin=-maxmag33, vmax=maxmag33)
plt.colorbar()
...
maxmag45 = np.amax(np.abs(heff45))
plt.imshow(heff45, cmap="PuOr", vmin=-maxmag45, vmax=maxmag45)
plt.colorbar()
...
from scipy.special import jv, jn_zeros
scale = jn_zeros(0,3)[-1]/21
plt.plot((s_list[1:]-0.5),-np.diag(heff45, k=1), label="Tunneling, +1")
plt.plot((s_list[1:]-0.5),-np.diag(heff45, k=-1), label="Tunneling, -1")
...
with mpl.rc_context(rc=talk_style):
    ...
    ax.plot(np.linspace(-22,22,256,endpoint=True), fit_amp*jv(0, np.linspace(-22,22,256,endpoint=True)*fit_scale), c=kelly_color_cycler.by_key()['color'][3], label="Bessel J0")
    ax.plot((s_list[1:]-0.5), -np.diag(heff45, k=1), '.')
    ax.plot(j33_list, -np.diag(heff33, k=1), '.')
    ax.set_xlabel('site index $j$')
    ax.set_ylabel('$-J_{j,j-1}/h$ (Hz)')
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'bessel_tun' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
print(fit_scale, fit_amp)
...
with mpl.rc_context(rc=talk_style):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'heff' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
...
with mpl.rc_context(rc=talk_style):
    fig, ax = plt.subplots(1,1, figsize=(1*2.75,2*2.75))
    ax.axis('off')
    plt.colorbar(mesh)
    fig.savefig(os.path.join(out_dir, 'heff_cb' + out_ext), dpi=out_dpi, bbox_inches='tight', pad_inches=1/72)
...
with mpl.rc_context(rc=talk_style):
    ...
    fig.tight_layout()
    fig.align_ylabels()
    fig.savefig(os.path.join(out_dir, 'modsqp_and_x' + out_ext), dpi=out_dpi)
...
with mpl.rc_context(rc=talk_style):
    ...
    group_panes = gridspec.GridSpec(3,1)
    for idx, this_set in enumerate([smooth_set, wiggle_set, other_set,
                                    resplit_set, again_set, rem_set][:3]):
        top, bottom = gridspec.GridSpecFromSubplotSpec(1,2,
                                                      subplot_spec=group_panes[idx],
                                                      wspace=0.1, hspace=0.0, width_ratios=[3,1])
        axt = fig.add_subplot(top)
    ...
    fig.tight_layout()
    fig.align_ylabels()
    fig.savefig(os.path.join(out_dir, 'modsqp_and_x_first3' + out_ext), dpi=out_dpi)
...
with mpl.rc_context(rc=talk_style):
    ...
    fig.tight_layout()
    fig.align_ylabels()
    fig.savefig(os.path.join(out_dir, 'modsqp_and_x_last3' + out_ext), dpi=out_dpi)
...
with mpl.rc_context(rc=talk_style):
    ...
    ax.set_xlabel('$\\gamma$ (Breathing Amp.)')
    plt.xlim(1e-5, 0.99)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'omega_crit_loglog' + out_ext), dpi=600)
...
with mpl.rc_context(rc=talk_style):
    ...
    plt.ylim(1e-6, 1e6)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'omega_drivemag_loglog' + out_ext), dpi=600)
...
with mpl.rc_context(rc=talk_style):
    ...
    plt.ylim(1e-6, 10)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'chi_fourier_loglog' + out_ext), dpi=600)
...
with mpl.rc_context(rc=talk_style):
    ...
    plt.ylim(1e-6, 10)
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'smallxi_fourier_loglog' + out_ext), dpi=600)
...
with mpl.rc_context(rc=talk_style):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, 'bigxi_fourier_loglog' + out_ext), dpi=out_dpi)
